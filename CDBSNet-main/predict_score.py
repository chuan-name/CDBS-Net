...
def evaluate(pred_path, gt_path, verbose=True):
    ...
    for dataset in datasets:
        ...
        preds = sorted([f for f in os.listdir(pred_root)
                        if os.path.isfile(os.path.join(pred_root, f)) and is_image_file(f)])
        gts = sorted([f for f in os.listdir(gt_root)
                      if os.path.isfile(os.path.join(gt_root, f)) and is_image_file(f)])
        if len(preds) != len(gts):
            raise ValueError(f"{dataset} 数据集中预测图像与GT图像数量不一致: preds={len(preds)}, gts={len(gts)}")
        preds.sort()
        gts.sort()
        ...
        for i, sample in samples:
            ...
            pred_path_full = os.path.join(pred_root, pred)
            gt_path_full = os.path.join(gt_root, gt)


            pred_mask = np.array(Image.open(pred_path_full))
            gt_mask = np.array(Image.open(gt_path_full))
            ...
            gt_mask = (gt_mask > 0).astype(np.float64)
...
if __name__ == "__main__":
    ...
    dataset_path_pre = './results/CDBSNet-best/'
